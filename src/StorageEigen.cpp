...
StorageVector::StorageVector(const StorageVector & other) :
        storage_(other.storage_), vector(storage_.head(other.vector.size())) {}

StorageVector & StorageVector::operator=(const StorageVector & other) {
    storage_ = other.storage_;
    new (&vector) Eigen::Ref<Vector>(storage_.head(other.vector.size()));
...
StorageMatrix2D::StorageMatrix2D(const StorageMatrix2D & other) :
        storage_(other.storage_), matrix(storage_.topRows(other.matrix.rows())) {}

StorageMatrix2D & StorageMatrix2D::operator=(const StorageMatrix2D & other) {
    storage_ = other.storage_;
    new (&matrix) Eigen::Ref<Matrix2D>(storage_.topRows(other.matrix.rows()));
