#ifndef MATPLOTLIB_HEADER_GUARD
#define MATPLOTLIB_HEADER_GUARD
...
PyObject* get_array(const std::vector<Numeric>& v)
{
    ...
    PyArrayObject* varray = (PyArrayObject*)PyArray_SimpleNew(1, &vsize, type);
    double * data = (double*)PyArray_DATA(varray);
...
bool plot(const std::vector<Numeric> &x, const std::vector<Numeric> &y, const std::map<std::string, std::string>& keywords)
{
    ...
    for(std::map<std::string, std::string>::const_iterator it = keywords.begin(); it != keywords.end(); ++it)
    {
        if (it->first == "dashes") {
            ...
            auto pieces = tokenize(s, ",");
            if (pieces.size() == 1) {
                PyDict_SetItemString(kwargs, it->first.c_str(), PyFloat_FromDouble(std::stod(pieces[0])));
            } else {
                PyObject* vals = PyTuple_New(pieces.size());
                for (size_t i = 0; i < pieces.size(); ++i) {
                    PyTuple_SetItem(vals, i, PyFloat_FromDouble(std::stod(pieces[i])));
                }
                PyDict_SetItemString(kwargs, it->first.c_str(), vals);
                Py_DECREF(vals);
            }
        } else {
             ...
             const std::unordered_map<std::string, std::string> & keywords = {})
{
    ...
    PyObject* kwargs = PyDict_New();
    // PyDict_SetItemString(kwargs, "s", zarray);
    for (const auto& it : keywords)
    {
        PyDict_SetItemString(kwargs, it.first.c_str(), PyString_FromString(it.second.c_str()));
    }
